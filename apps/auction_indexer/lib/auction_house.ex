defmodule AuctionIndexer.AuctionHouse do
  ...
  def process(state, [] = _messages), do: state
  def process(state, [next|rest] = _messages) do
  ...
  defp process_event(state, {:new, new_auction} = _event, _existing_auction) do
    update_record(state, new_auction, %{})
  end
  defp process_event(state, _parsed_event, nil), do: state
  defp process_event(state, {:bid,    changeset}, existing_auction) do
    update_record(state, existing_auction, changeset.changes)
  end
  defp process_event(state, {:buyout, changeset}, existing_auction) do
    update_record(state, existing_auction, changeset.changes)
  end
  defp process_event(state, {_,   changeset}, existing_auction) do
    add_done_auction(state, existing_auction, changeset.changes)
  end
